...
function chooseScript(){
    case $PEER_INDEX in
        1)  {
            sleep 1s
            /bin/bash /chain/chaintest-yuelian/fabric/start/fabric_orderer_start.sh 
        }
        ;;
        2)  {
            sleep 2s
            /bin/bash /chain/chaintest-yuelian/fabric/start/fabric_ca1_start.sh 
        }
        ;;
        3|4|5|6)  {
            sleep $PEER_INDEX
            /bin/bash /chain/chaintest-yuelian/fabric/start/fabric_peer_start.sh 
        }
        ;;
        *)  echo 'error'
        ;;
