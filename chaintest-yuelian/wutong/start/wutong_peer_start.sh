...
echo "peer$PEER_INDEX.example.com" > /chain/CONTAINER_NAME
ls /var/lib/docker/containers | grep `docker ps -a | grep " peer$PEER_INDEX.example.com" | awk '{print $1}'` > /chain/CONTAINER_ID
echo "success"